...
export async function onRequestGet(context) {
  ...
  const game = requestUrl.searchParams.get("game") || DEFAULT_GAME;
  const week = requestUrl.searchParams.get("week") || DEFAULT_WEEK;
  ...
  try {
    ...
    return Response.json(
      { error: "Could not load stored Game 1 results", detail: error.message },
      { status: 502 }
    );
  }
}
